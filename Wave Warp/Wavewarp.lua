...
if not sprite then return app.alert("No active sprite found! Please open the sprite you want to modify. :)") end
    ...
    if not layer or layer.isBackground then
        ...
        if not activeCel then
            ...
            if not activeCel then return end
                ...
                local pFrame = (args.previewFrame or 1) - 1
                local img, pad = getWarpedImage(originalImage, pFrame, args)
                ...
                dlg:separator{ text="Secondary Wave" }
                ...
                dlg:number{ id="frames", label="Frame Count:", text="16", decimals=0, onchange=function()
                    local maxFrames = math.max(1, dlg.data.frames or 1)
                    dlg:modify{ id="previewFrame", max=maxFrames }
                    updatePreview()
                    end }
                    dlg:slider{ id="previewFrame", label="Scrub Preview:", min=1, max=16, value=1, onchange=updatePreview }
